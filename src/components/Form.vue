...
<form v-cloak>
    ...
    <ul>
        ...
        <li v-for="facility in facilities" v-bind:class="{'active' : facility.active}" :key="facility.name" @click="selectProduct(facility)">
            <!-- Display the name and price for every entry in the array .
            Use the formatCurrency method for formatting the price -->
            {{facility.name}} <span>{{this.formatCurrency(facility.price)}}</span>
        </li>
    </ul>
...
export default{
    ...
    methods: {
        ...
        total: function(){
            ...
            var total = 0;
            this.facilities.forEach((product)=>{
